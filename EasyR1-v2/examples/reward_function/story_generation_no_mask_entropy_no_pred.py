...
def compute_score(
    ...
    """
    Compute reward scores based on Contextual Reasoning Entropy (CRE).
    ...
    for reward_input in reward_inputs:
        ...
        d_i = torch.clamp(raw_d_i, min=-10.0, max=10.0)

        # Compute weights alpha_i
        raw_alpha_i = (1.0 / T) * (1.0 + beta * d_i + gamma * info_flow)
        
        # Normalize weights
        sum_alpha = torch.sum(raw_alpha_i)
        
        if abs(sum_alpha) < epsilon:
            alpha_prime = torch.ones_like(raw_alpha_i) / T
        else:
            alpha_prime = raw_alpha_i / sum_alpha
        ...
        if use_format_gate:
            ...
            # if global_step > 25 and r_format > 0.95:
            #     format_weight = 0.1
        ...
        scores.append({
            ...
            "min_alpha": torch.min(alpha_prime).item(),
