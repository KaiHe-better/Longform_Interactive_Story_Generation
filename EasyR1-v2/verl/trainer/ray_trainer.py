...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        # 添加最好的章节跳转准确率
        self.val_acc_next_episode = 0.0
        self.best_val_acc_next_episode = -1.0
        self.best_global_step = None
        self.best_acc_step = None
        ...
        if config.trainer.max_steps is not None:
            ...
            self.training_steps = (
                num_examples
                // config.data.rollout_batch_size
    ...
    #         "best_val_acc_next_episode": round(self.best_val_acc_next_episode, 4),
    #         "best_acc_step": self.best_acc_step,
    #     }
    #     checkpointer_tracker_path = os.path.join(
    #         self.config.trainer.save_checkpoint_path, CHECKPOINT_TRACKER
    #     )
    #     with open(checkpointer_tracker_path, "w") as f:
    #         json.dump(checkpointer_tracker_info, f, ensure_ascii=False, indent=2)

    # def _save_checkpoint(self, is_periodic: bool, is_best_reward: bool, is_best_acc: bool) -> None:
    #     # 保存当前checkpoint
    #     folder_path = os.path.join(
    #         self.config.trainer.save_checkpoint_path, f"global_step_{self.global_step}"
    #     )
    ...
    #             self.config.trainer.save_checkpoint_path, "best_reward", "actor"
    #         )
    #         self.actor_rollout_ref_wg.save_checkpoint(
    #             best_reward_path, save_model_only=self.config.trainer.save_model_only
    #         )
        
    #     # 更新最佳accuracy记录
    #     if self.val_acc_next_episode > self.best_val_acc_next_episode:
    #         self.best_val_acc_next_episode = self.val_acc_next_episode
    #         self.best_acc_step = self.global_step
            
    #         # 额外保存最佳accuracy模型到特殊目录
    #         best_acc_path = os.path.join(
    #             self.config.trainer.save_checkpoint_path, "best_accuracy", "actor"
    #         )
    #         self.actor_rollout_ref_wg.save_checkpoint(
    #             best_acc_path, save_model_only=self.config.trainer.save_model_only
    #         )
    ...
    #     # 清理旧checkpoint（保留best_global_step和best_acc_step）
    #     remove_obsolete_ckpt(
    #         self.config.trainer.save_checkpoint_path,
    #         self.global_step,
    #         [self.best_global_step, self.best_acc_step],  # 保留两个最佳步骤
    #         self.config.trainer.save_limit,
    #     )
    ...
    #         "last_global_step": self.global_step,
    #         "last_actor_path": os.path.abspath(actor_path),
    #         "best_reward_path": os.path.abspath(os.path.join(
    #             self.config.trainer.save_checkpoint_path, "best_reward", "actor"
    #         )),
    #         "best_acc_path": os.path.abspath(os.path.join(
    #             self.config.trainer.save_checkpoint_path, "best_accuracy", "actor"
    #         )),
    #     }
    #     checkpointer_tracker_path = os.path.join(
    ...
    #     with open(checkpointer_tracker_path, "w") as f:
    #         json.dump(checkpointer_tracker_info, f, ensure_ascii=False, indent=2)
    def _save_checkpoint(self, is_periodic: bool = True, is_best_reward: bool = False, is_best_acc: bool = False) -> None:
        folder_path = os.path.join(
            self.config.trainer.save_checkpoint_path, f"global_step_{self.global_step}"
        )
        actor_path = os.path.join(folder_path, "actor")
        ...
        if is_periodic:
            # 只有在周期性保存时，才创建 global_step_xxx 文件夹并保存全部内容
            self.actor_rollout_ref_wg.save_checkpoint(
                actor_path, save_model_only=self.config.trainer.save_model_only
            )
        ...
        #         best_reward_path, save_model_only=self.config.trainer.save_model_only
        #     )
        #     print(f"Update best reward model at step {self.global_step} with score {self.best_val_reward_score:.4f}")
        ...
        #         best_acc_path, save_model_only=self.config.trainer.save_model_only
        #     )
        #     print(f"Update best accuracy model at step {self.global_step} with acc {self.best_val_acc_next_episode:.4f}")

        if is_periodic:
            remove_obsolete_ckpt(
                self.config.trainer.save_checkpoint_path,
                self.global_step,
                # 注意：如果 best step 不是周期性保存的 step，这个文件可能在 global_step_xxx 中不存在
                [self.best_global_step, self.best_acc_step], 
                self.config.trainer.save_limit,
            )
        ...
        checkpointer_tracker_info = {
            ...
            "best_val_acc_next_episode": round(self.best_val_acc_next_episode, 4),
            "last_global_step": self.global_step,
            # 注意：last_actor_path 可能指向一个不存在的路径（如果这一步只是best update而不是periodic）
            # 建议只有 is_periodic 为 True 时才更新 last_actor_path，或者保持原样指向最新的一次
            "last_actor_path": os.path.abspath(actor_path), 
            "best_reward_path": os.path.abspath(os.path.join(
                self.config.trainer.save_checkpoint_path, "best_reward", "actor"
            )),
    ...
    def _load_checkpoint(self) -> None:
        print(f"self.config.trainer.find_last_checkpoint: {self.config.trainer.find_last_checkpoint}")
        if self.config.trainer.load_checkpoint_path is not None:
            ...
            load_checkpoint_path, tracker_info = find_latest_ckpt(
                self.config.trainer.save_checkpoint_path
            )
            if tracker_info is not None:
                self.best_val_reward_score = tracker_info.get(
        ...
        if (
            "global_step_"
            not in load_checkpoint_path.strip(os.path.sep).split(os.path.sep)[-1]
        ):
            raise ValueError("`load_checkpoint_path` should end with `global_step_*`.")

        print(f"Load from checkpoint: {load_checkpoint_path}.")
        self.global_step = int(
            load_checkpoint_path.strip(os.path.sep).split("global_step_")[-1]
        )
        actor_path = os.path.join(load_checkpoint_path, "actor")
        print(f"load actor from path: {actor_path}")
    ...
    def _validate(self) -> dict[str, Any]:
        ...
        non_tensor_batch_keys = ["raw_prompt_ids", "raw_prompt_ids_without_r"]
        ...
        for batch_dict in self.val_dataloader:
            ...
            test_output_gen_batch = self.actor_rollout_ref_wg.generate_sequences(
                test_gen_batch
            )
            ...
            # validate 也需要计算 Logprobs 
            test_log_probs = self.actor_rollout_ref_wg.compute_log_probs(test_batch)
            test_log_probs.meta_info = {}
            test_batch = test_batch.union(test_log_probs)
            ...
            if "val/acc_next_episode" in reward_metrics:
                acc_lst.extend(reward_metrics["val/acc_next_episode"])
        ...
        self.val_reward_score = (
            torch.cat(reward_tensor_lst, dim=0).sum(-1).mean().item()
        )
        self.val_acc_next_episode = sum(acc_lst) / len(acc_lst) if acc_lst else 0.0
    ...
    def _make_batch_data(self, metrics: dict[str, Any]) -> DataProto:
        ...
        while True:
            ...
            new_batch: DataProto = DataProto.from_single_dict(
                batch_dict, meta_info=meta_info
            )
            ...
            # 新增了一些key
            # 这些内容在 EasyR1/verl/utils/dataset.py: def __getStoryItem__(self, index):修改
            batch_keys = [
                "input_ids",
                "attention_mask",
                "position_ids",
            ...
            if self.config.algorithm.adv_estimator == "remax":
                ...
                gen_baseline_output = self.actor_rollout_ref_wg.generate_sequences(
                    gen_baseline_batch
                )
            ...
            if self.config.algorithm.online_filtering:
                ...
                kept_sample_idxs = [
                    idx for idx, uid in enumerate(uids) if uid in kept_uids
                ]
                if len(kept_sample_idxs) == 0:
                    raise RuntimeError(
                        "No sample is kept after filtering. Please check your data."
                    )

                new_batch = new_batch[kept_sample_idxs]

            batch = (
                DataProto.concat([batch, new_batch]) if batch is not None else new_batch
            )
            ...
            if current_batch_size < rollout_batch_size:
                ...
                print(
                    f"{current_batch_size=} >= {rollout_batch_size=}. Finish generating."
                )
                if self.config.algorithm.online_filtering:
                    metrics.update(
                        {
                            f"reward/{k}": v
                            for k, v in reduce_metrics(all_metrics).items()
                        }
                    )
    ...
    def fit(self):
        ...
        while self.global_step < self.training_steps:
            ...
            with timer("step", timing_raw):
                ...
                # 并且在这里需要计算由 old_log_prob 得到的熵作为奖励信号传进 reward_function 中
                with timer("old", timing_raw):
                    # 实际上，这里的 old_log_probs 还塞了一个 entropy 和 info_flow
                ...
                if self.use_reference_policy:
                    with timer("ref", timing_raw):
                        ref_log_probs = self.actor_rollout_ref_wg.compute_ref_log_probs(
                            batch
                        )
                ...
                #     and self.global_step % self.config.trainer.save_freq == 0
                # ):
                if is_periodic or is_best_reward or is_best_acc:
                    with timer("save_checkpoint", timing_raw):
                        self._save_checkpoint(is_periodic, is_best_reward, is_best_acc)
