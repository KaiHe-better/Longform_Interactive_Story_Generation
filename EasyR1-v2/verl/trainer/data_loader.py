...
def create_dataloader(config: DataConfig, tokenizer: PreTrainedTokenizer, processor: Optional[ProcessorMixin]) -> None:
    ...
    if config.cut_dataset:
        max_train_samples = config.max_train_samples
        if max_train_samples != -1:

            if len(train_dataset) > max_train_samples:
                print(f"原始训练集大小: {len(train_dataset)}，正在截断至: {max_train_samples}")
                
                g = torch.Generator()
                g.manual_seed(config.seed)
                
                indices = torch.randperm(len(train_dataset), generator=g)[:max_train_samples].tolist()
    ...
    if config.cut_dataset:
        ...
        if max_val_samples != -1:
            if len(val_dataset) > max_val_samples:
                print(f"原始验证集大小: {len(val_dataset)}，正在截断至: {max_val_samples}")
