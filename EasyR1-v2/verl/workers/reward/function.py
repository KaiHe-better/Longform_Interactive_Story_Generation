...
class BatchFunctionRewardManager(FunctionRewardManager):
    ...
    def compute_reward(self, data: DataProto) -> Tuple[torch.Tensor, dict[str, list[float]]]:
        ...
        response_ids = data.batch["responses"]
        response_mask = data.batch["response_mask"] # [Batch, Seq_Len]
        response_length = torch.sum(response_mask, dim=-1)
        
        tensor_keys = ['old_log_probs', 'old_entropy', 'info_flow', 'entropy_wo_r']
        ...
        for i in range(len(data)):
            ...
            for key in tensor_keys:
                if key in data.batch:
                    valid_tensor = data.batch[key][i][:cur_response_length]
                    input_dict[key] = valid_tensor
                else:
                    print(f"Warning: {key} not found in batch keys: {data.batch.keys()}")
