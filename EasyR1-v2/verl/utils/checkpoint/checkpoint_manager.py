...
def remove_obsolete_ckpt(
    path: str, 
    global_step: int, 
    best_global_steps: list[int],  # 改为列表
    save_limit: int = -1, 
    ...
    # 保护所有best steps
    protected_steps = []
    for best_step in best_global_steps:
        if best_step in ckpt_global_steps:
            ckpt_global_steps.remove(best_step)
            protected_steps.append(best_step)
    
    # 减少可保留的普通checkpoint数量
    num_ckpt_to_keep = max(num_ckpt_to_keep - len(protected_steps), 0)
